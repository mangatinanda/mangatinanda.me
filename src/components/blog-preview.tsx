        ...
        {blogPosts.map((post, i) => (
          ...
          >
            {post.published ? (
              ...
              >
                <PreviewCardBody post={post} />
              </Link>
            ) : (
              <div className="block p-6 rounded-xl bg-surface border border-border-color opacity-60 h-full">
                <PreviewCardBody post={post} />
              </div>
            )}
          </motion.div>
...
function PreviewCardBody({ post }: { post: BlogPost }) {
  return (
    <>
